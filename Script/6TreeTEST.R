...
df <- read_sav("pisa_data/2022/InitialDatabasePerLanguage.sav")
# Split data
set.seed(123)
...
# num.trees = 10p, mtry = 41, min.node.size = 5, sample.fraction = .80, replace = F
...
#printing 25 most important features
vip::vip(rf_optim,    num_features = 25, bar = FALSE)

#predicted values of training and testing model
...
hyper_grid <- expand.grid(
  ...
  interaction.depth = c(1, 3, 5, 17),    # from the hyper grid output 
...
pdf("true_vs_predicted_plots_Language_test.pdf")
